...
function WorkoutHistory() {
  ...
  const fetchAllActivities = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('http://localhost:8081/activity', {
        headers: {
  ...
  const fetchByDate = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`http://localhost:8081/activity/byDate?date=${filterDate}`, {
        headers: {
          'Authorization': 'Bearer ' + token,
          'Content-Type': 'application/json'
        }
  ...
  const handleSave = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:8081/activity/${id}`, editData, {
        headers: {
          'Authorization': 'Bearer ' + token,
          'Content-Type': 'application/json'
        }
